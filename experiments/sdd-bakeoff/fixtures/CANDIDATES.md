...
| `lint+`     | upper           | `lint` grown per its own Growth note: 5+ rules, worker-pool dispatch, hash-keyed result cache, config inheritance — the step toward extracting `danhorst/lint`. | `ok/bad/fixed` per-rule fixtures | fixture written; run per `seed-lint+.md` (four cells + thesis implementer sweep) when usage limits reset |
