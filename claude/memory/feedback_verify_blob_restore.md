...
The dangling blob I matched by diff-shape and restored turned out to be a pre-June-16 version: it reintroduced a retired `UserPromptSubmit` sync hook (firing an error every prompt) and silently dropped the current `SessionStart` `memory seed` hook, the `PreToolUse` hook, and a permission.
