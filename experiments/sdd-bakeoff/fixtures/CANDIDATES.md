...
| ----------- | --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- | ---------------------------------------------------------------------------------- |
| `json-sort` | lower           | Go rewrite of `bin/json-sort` (sort JSON keys in place, preserve mode, multi-file, missing-dep and not-a-file errors).                                          | `ok/bad/fixed` sorted JSON       | next — small enough that quality should saturate, which is the lower-bound finding |
| `lint+`     | upper           | `lint` grown per its own Growth note: 5+ rules, worker-pool dispatch, hash-keyed result cache, config inheritance — the step toward extracting `danhorst/lint`. | `ok/bad/fixed` per-rule fixtures | queued — run with the implementer sweep when usage limits reset                    |
